...
#                  oled.show()
        ...
        oled.fill(0)
        oled.text("Measurement 1", 5, 25, 1)
        oled.show()
        time.sleep(2)
        oled.fill(0)
        ...
        try:
            # opens 'sample.json' in read mode ('r')
            ...
            if "basic_hrv" in json_object and isinstance(json_object.get("basic_hrv"), dict):
                ...
                oled.text(f"PPI:{basic_hrv_data.get('mean_ppi')} HR:{basic_hrv_data.get('mean_hr')}", 5, 25, 1) 
                oled.text(f"SDNN:{basic_hrv_data.get('sdnn')}", 5, 35, 1)
                oled.text(f"RMSSD:{basic_hrv_data.get('rmssd')}", 5, 45, 1)
...
#                 print("Analysis data not") 
# 
#             else:
#                print("History file format")
#                
# 
        except:
            ...
            try:
                ...
                if access_token:
                    ...
                    if analysis_data.get("status") == "ok" and "analysis" in analysis_data:
                        ...
                        oled.text(f"Mean HR: {analysis.get('mean_hr_bpm', 0.0):.1f}bpm", 5, 0, 1)
                        oled.text(f"Mean PPI: {analysis.get('mean_rr_ms', 0.0):.1f}ms", 5, 10, 1)
