...
def read_adc(tid):
    ...
    samples.put(x)
    data.append(x)
    rr_intervals.append(x)
    
def capture_heart_rate():
    global samples, rr_intervals, data
    rr_intervals = []
    data = []

    oled.fill(0)
    oled.text("Measuring HR...", 0, 20)
    oled.show()

    # Set up everything needed (e.g., timers, buffers)
    # *** Move your sampling code here ***
    
    tmr = Timer(freq = samplerate, callback = read_adc)
    
    # Sampling loop
    capture_count = 0
    while capture_count < samplerate * 10:  # measure for 10 seconds
        if not samples.empty():
            x = samples.get()
            capture_count += 1
            # Optional: simple processing to find peaks and build rr_intervals
            # You can simplify and assume every sample is an RR interval for now

    tmr.deinit()
    oled.fill(0)
    oled.text("Done Measuring", 0, 20)
    oled.show()
    utime.sleep(1)

    # After capturing, you now have new rr_intervals!
...
def Measure_HR(rr_intervals):
    meanPPI = meanPPI_calculator(rr_intervals)
    oled.fill(0)
...
def Basic_HRV_analysis(rr_intervals):
    meanPPI = meanPPI_calculator(rr_intervals)
    result = RMSSD_calculator(rr_intervals)
    oled.fill(0)
    oled.text("HRV Analysis:",0, 10)
    oled.show()
    return result
...
def History(rr_intervals):
    ...
    meanPPI = meanPPI_calculator(rr_intervals)
    result = SDNN_calculator(rr_intervals, meanPPI)
...
meanPPI = meanPPI_calculator(rr_intervals)
Measure_HR(rr_intervals)
Basic_HRV_analysis(rr_intervals)
...
def select_option():
    ...
    if prev_push == 1 and current_push == 0:  # buttonpressed
        ...
        if selected_option == "Measure HR ":
            ...
            capture_heart_rate()
            result = Measure_HR(rr_intervals)
            oled.fill(0)
