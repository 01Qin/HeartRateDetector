...
def print_kubios_summary(response):
    if response.get("status") != "ok":
        print("Analysis failed or returned non-ok status.")
        return
...
#                  oled.show()
      ...
      # Attempt to load history data from 'sample.json'
        try:
            # Use a 'with' statement for safe file handling; opens 'sample.json' in read mode ('r')
            with open('sample.json', 'r') as openfile:
                json_object = ujson.load(openfile) # Use ujson for MicroPython compatibility

            # Check if the loaded data is a valid dictionary and contains expected main keys
            if json_object and isinstance(json_object, dict) and "type" in json_object and "data" in json_object:
                # Extract basic information using .get() for safety (provides default if key is missing)
                data_type = json_object.get("type", "N/A")
                
                data_count = len(json_object.get("data", [])) if isinstance(json_object.get("data"), list) else 0
                timestamp = json_object.get("timestamp")


               

                basic_hrv_found = False
                kubios_found = False

                # --- Display Basic HRV results if available ---
                # Check if the 'basic_hrv' nested dictionary exists and is a dictionary
            if "basic_hrv" in json_object and isinstance(json_object.get("basic_hrv"), dict):
                basic_hrv_data = json_object["basic_hrv"] # Use a distinct variable name
                basic_hrv_found = True

                oled.text("Basic HRV:", 5, 35, 1) # Add a header for basic HRV results
                oled.text(f"PPI:{basic_hrv_data.get('mean_ppi', 'N/A')} HR:{basic_hrv_data.get('mean_hr', 'N/A')}", 5, 45, 1) # Mean PPI and Mean HR
                     # Corrected: Removed duplicate RMSSD line. Display SDNN and RMSSD on one line.
                oled.text(f"SDNN:{basic_hrv_data.get('sdnn', 'N/A')} RMSSD:{basic_hrv_data.get('rmssd', 'N/A')}", 5, 55, 1)
                     # SD1 and SD2 might need a different layout or line depending on your screen

                # Check if the 'analysis' nested dictionary and 'readiness' within it exists
            if "analysis" in json_object and isinstance(json_object.get("analysis"), dict) and "readiness" in json_object.get("analysis", {}):
                kubios_readiness_data = json_object["analysis"].get("readiness", {}) # Use a distinct variable name
                     # Check if key Kubios readiness parameters are actually present
                if 'pns_index' in kubios_readiness_data and 'sns_index' in kubios_readiness_data:
                    kubios_found = True
                    oled.text("Kubios:", 65, 35, 1) # Add a header for Kubios results (adjust position if needed)

                         # Display key Kubios readiness parameters
                    pns_val = kubios_readiness_data.get('pns_index', 'N/A')
                    sns_val = kubios_readiness_data.get('sns_index', 'N/A')
                    readiness_score_val = kubios_readiness_data.get('readiness_score', 'N/A')

                         # Display PNS/SNS and Readiness Score (adjust positions to fit)
                    oled.text(f"PNS:{pns_val} SNS:{sns_val}", 65, 45, 1)
                    oled.text(f"Readiness: {readiness_score_val}", 65, 55, 1)


                # Message if neither Basic HRV nor Kubios analysis data sections are found in the file
            if not basic_hrv_found and not kubios_found:
                oled.text("Analysis data not", 5, 35, 1) # Display message if no analysis data found
                oled.text("in file.", 5, 45, 1)

            else:
                oled.text("History file format", 5, 20, 1)
                oled.text("error.", 5, 30, 1)


        except:
            # Specific error handling if 'sample.json' does not exist
            oled.text("History file not", 5, 20, 1)
            oled.text("found.", 5, 30, 1)


        oled.show() # Update the physical OLED display with all the text drawn
        ...
        if 'last_ppi_array' in globals() and len(last_ppi_array) >= 3:
            ...
            try:
                ...
                if access_token:
                    ...
                    oled.fill(0) # Clear screen for results
                    oled.text("Kubios Results:", 5, 0, 1)

                    if analysis_data.get("status") == "ok" and "analysis" in analysis_data:
                        analysis = analysis_data["analysis"]
                        # Display key metrics on OLED
                        oled.text(f"Mean HR: {analysis.get('mean_hr_bpm', 0.0):.1f}bpm", 5, 10, 1)
                        oled.text(f"Mean PPI: {analysis.get('mean_rr_ms', 0.0):.1f}ms", 5, 20, 1)
                        oled.text(f"SDNN: {analysis.get('sdnn_ms', 0.0):.1f}ms", 5, 30, 1) # Adjusted y
                        oled.text(f"RMSSD: {analysis.get('rmssd_ms', 0.0):.1f}ms", 5, 40, 1) # Adjusted y         
                        oled.text(f"PNS:{analysis.get('pns_index', 0.0):.2f}SNS:{analysis.get('sns_index', 0.0):.2f}", 5, 50, 1)
                    else:
                        oled.text("Analysis Failed", 5, 15, 1)
                        oled.text("Check console", 5, 25, 1) # Hint to check print output

                else:
                    oled.text("Failed to get", 5, 15, 1)
                    oled.text("access token", 5, 25, 1)


            except Exception as e:
                oled.fill(0)
                oled.text("Kubios Error:", 5, 5, 1)
                # Attempt to display part of the error, may be too long
                error_str = str(e)
                oled.text(error_str[:16], 5, 20, 1) # Display first part
                if len(error_str) > 16:
                    oled.text(error_str[16:32], 5, 30, 1) # Display second part
                oled.text("Check console", 5, 45, 1)
                print("Kubios Analysis Error:", e) # Print full error to console for debugging

        else:
            oled.text("No data for", 5, 20, 1)
            oled.text("Kubios analysis.", 5, 30, 1)
            oled.text("Perform HR", 5, 40, 1)
            oled.text("measurement first.", 5, 50, 1)
